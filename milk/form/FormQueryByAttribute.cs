...
namespace QueryAndStatistics
{
    public partial class FormQueryByAttribute : Form
    {
           ...
           // try
            {
                ...
                while (row != null)
                {
                    if (index == 0)
                    {
                        listBoxValues.Items.Add(row.get_Value(index).ToString());
                    }
                    else
                    {
                        ...
                        if (field.Type == esriFieldType.esriFieldTypeString)
                        {
                            //listBoxValues.Items.Add("\'" + row.get_Value(index + 1).ToString() + "\'");
                            if (!listBoxValues.Items.Contains(row.get_Value(index + 1).ToString()))
                            {
                                listBoxValues.Items.Add("\'" + row.get_Value(index + 1).ToString() + "\'");
                            }
                           
                            // listBoxValues.Items.Add("\'" + row.get_Value(2).ToString() + "\'");
        ...
        public int get_string_index(IRow row)
        {
            int string_index;
            string_index = listBoxFields.SelectedIndex;
            int j;
        
                j = row.Fields.FieldCount;
                for (int i = 0; i < j; i++)
                {
                    string selected_item_string = listBoxFields.SelectedItem.ToString();
                    string field_string = row.Fields.get_Field(i).ToString();
                    if (selected_item_string == row.Fields.get_Field(i).ToString())
                    {
                        return i;
